...
| **Business Critical** | **Private Link**, 고도의 암호화 등 최고 수준의 보안 |
...
Snowflake는 **역할 기반 제어(RBAC)** 를 사용하며, 상위 역할이 하위 역할의 권한을 상속받는 구조를 취합니다. **ACCOUNTADMIN** 은 계정 내 최상위 권한으로, 보안 설정과 비용 모니터링을 총괄합니다.
...
| **PUT 명령어** | 로컬 파일 업로드 시 **SnowSQL(CLI)** 에서만 사용 가능 (Snowsight UI 불가) |
| **COPY INTO** | 벌크 로드용 |
...
반정형 데이터는 **VARIANT 타입(최대 16MB)** 으로 저장되며, **FLATTEN 함수**를 통해 관계형 구조로 변환됩니다.
...
| **Reader Account** | Snowflake 계정이 없는 파트너에게 제공. **컴퓨팅 비용은 공급자(Provider) 부담** |
| **Snowflake Marketplace** | ETL 과정 없이 외부 데이터를 즉시 구독하여 분석 가능 |
| **Replication / Failover** | 계정 간 복제 및 장애 조치로 비즈니스 연속성 보장 |

> **공유 가능한 오브젝트**: 일반 테이블, 외부 테이블, 보안 뷰(Secure View), 보안 구체화된 뷰, 보안 UDF
...
> **정답: A, C** — Snowpark는 Python, Java, Scala 세 가지 언어를 공식 지원합니다. 주어진 보기 중 Python(A)과 Java(C)가 정답이며, Scala도 지원하나 보기에 없습니다.
...
- B. SnowSQL(CLI) 또는 JDBC/ODBC 드라이버 등의 커넥터를 통해서만 실행 가능하다.
...
> **정답: B, C** — 데이터 언로딩(`COPY INTO <location>`) 시 지원되는 반정형 형식은 JSON과 Parquet입니다. ORC(D)는 데이터 로딩(`COPY INTO <table>`) 시에만 지원되며, 언로딩에는 사용할 수 없습니다.
...
> **정답: B, D** — 데이터 언로딩(`COPY INTO <location>`) 시 지원되는 반정형 형식은 JSON과 Parquet입니다. ORC(A)는 데이터 로딩(`COPY INTO <table>`) 시에만 지원되며, 언로딩에는 사용할 수 없습니다.
...
### 78. 반정형 데이터를 저장하는 VARIANT 데이터 타입이 한 행(Row)에 저장할 수 있는 최대 크기는 얼마인가?
...
> **정답: A, C** — 이 외에도 외부 테이블, 보안 구체화된 뷰, 보안 UDF 등을 공유할 수 있습니다.
